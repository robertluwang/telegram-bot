...
3. Find and download **Telegram** and **iSH Shell**.
   *(Note: You must have "purchased" these previously on a newer device using the same Apple ID).*
...
```bash
litellm --host 100.x.x.x --port 4000
```
...
On your local machine or a VM, create a file named `main.go`. This has already been done in the repository.
...
```ini
[Unit]
Description=Telegram Bot AI Bridge
...
Environment="LITELLM_URL=http://100.x.x.x:4000/v1/chat/completions"
Environment="LITELLM_KEY=sk-1234abcd..."
ExecStart=/path/to/bot/telegram-bot
Restart=always
...
1. Open the Telegram app on your iOS 12 iPad.
2. Send a message to your bot.
3. The message traverses: `Telegram's servers` -> `Your Golang binary (iSH or VM 1)` -> `Tailscale Encrypted Mesh` -> `LiteLLM Gateway (VM 2)` -> `Gemini API`.
